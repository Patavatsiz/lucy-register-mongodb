...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    // kayıtlı mı ?
    if (member.roles.cache.has(value.kadın1) || member.roles.cache.has(value.kadın2)) return message.react(value.emojiHayir)
    if (member.roles.cache.has(value.kadın1) && member.roles.cache.has(value.kadın2) && member.roles.cache.has(value.Kayıtsız)) return member.roles.remove(value.Kayıtsız);
    //kayıtlı mı ?
    ...
    member.roles.add(value.kadın1)
    member.roles.add(value.kadın2)
    ...
    // db ekleme
    let userdata = await new User({ User: member.id, Name: name, Gender: "Kadın"}); userdata.save().catch(err => { })
    ...
    if (checkstaffdata) {
      ...
      checkstaffdata.Female++
      checkstaffdata.save().catch(err => { })
    } else {
      let newstaffdata = await new Staff({ Author: message.author.id, Total: 1, Male: 0, Female: 1 }); newstaffdata.save().catch(err => { })
    }
    //db ekleme son
    ...
    let sta = await Staff.find({ Author: message.author.id }, async (err, data) => {
      ...
      let sdata = csdata.map(x => `__**Yetkilinin toplam kayıtları:**__ **${x.Total}**\n__**Yetkilinin toplam kadın kayıtları:**__ **${x.Female}**`)

      fembed("RANDOM", `${member} başarıyla <@&${value.kadın1}> olarak kayıt edildi\n\n${sdata}`)
      message.react(value.emojiEvet)

      lembed("GREEN", `${member} (\`${member.id}\`) ${message.author} (\`${message.author.id}\`) tarafından <@&${value.kadın1}> olarak kayıt edildi\nKayıt Tarihi: \`${moment(Date.now()).locale("tr").format("LLL")}\``)
    })
